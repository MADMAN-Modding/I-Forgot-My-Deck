...
use crate::{
    ...
    database::{
        self, add_account, check_account_exists, check_token, get_account, reset_token_time,
    },
    state::AppState,
};
...
pub async fn make_account(
    ...
    if !check_account_exists(&state.database, &account).await {
        ...
        return Err((
            ...
            axum::Json("Error Making Account".to_string()),
        ));
    }
    ...
    database::insert_struct(&state.database, code_struct)
        .await
        .unwrap();
...
pub async fn auth_account(
    Path((id, pass)): Path<(String, String)>,
    State(state): State<Arc<AppState>>,
) -> Result<(StatusCode, Json<Value>), (StatusCode, Json<Value>)> {
    let id = id.to_lowercase();

    let account = get_account(&state.database, &id).await.map_err(|_| {
        (
            StatusCode::BAD_REQUEST,
            Json(json!({"msg":"Account doesn't exist or invalid login credentials"})),
        )
    })?;
    ...
    if pass == hash_pass {
        if account.verified {
            ...
            match database::insert_struct(&state.database, token.clone()).await {
                Ok(_) => {}
                Err(e) => {
                    println!("{e:?}");
                    ...
                    return Err((
                        StatusCode::INTERNAL_SERVER_ERROR,
                        Json(json!({"msg": "failed generating token"})),
                    ));
                }
            };
            ...
            Err((
                StatusCode::BAD_REQUEST,
                Json(
                    json!({"msg":"Account not yet verified, please check your email, including the spam and trash."}),
                ),
            ))
        }
    } else {
